...
namespace App\Containers\UserSection\User\UI\API\Tests\Functional;

use App\Containers\UserSection\User\UI\API\Tests\ApiTestCase;
use Hashids;
...
class FindUserByIdTest extends ApiTestCase
{
    ...
    {
        $user = $this->getTestingUser();
        $user->assignRole(config('userSection-authorization.admin_role'));
        ...
        $response->assertJson(
            ...
            $json->has('data')
                ...
                ->where('data.roles.data.0.name', config('userSection-authorization.admin_role'))
                ->etc()
        );
    }
}
