...
class CustomImageUpload extends SpatieMediaLibraryFileUpload
{
    protected string $view = 'forms.components.spatie-image-upload';
    protected string $wrapperClass;

    protected bool $withSizes = false;
    ...
    protected function setUp(): void
    {
        ...
        $this->saveUploadedFileUsing(static function (SpatieMediaLibraryFileUpload $component, TemporaryUploadedFile $file, ?Model $record, callable $get): string {
            ...
            if ($component->isWithSizes()) {
                $imageInstance = InterventionImage::make($file);
                $imageInstance->orientate();
                $customProperties['width'] = $imageInstance->getWidth();
                $customProperties['height'] = $imageInstance->getHeight();
            }
    ...
    {
        return $this->evaluate($this->withSizes) ?? false;
    }
